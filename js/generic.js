...
var sNotification = new function () {
    ...
    $(function () {
        ...
        if (!_self.element.length) {
            var html = '<div class="statusbar hidden" style="background" id="myNotification">'
                ...
                + '</div>';
            _self.element = $(html).appendTo('body');
        }
    });
    ...
    this.loader = function (text, detail) {
            ...
            $({deg: start}).animate({deg: end}, {
                ...
                complete: function () {
                    if (_self.element.hasClass("statusbar-loader")) {
                        animateRotate(0, 360, 1000, 'linear');
                    } else if (!object.hasClass('stopAnimeOne')) {
                        animateRotate(0, 360, 3000, 'easeOutQuint');
        ...
        animateRotate(0, 360, 1500, 'easeInCubic');
    };
    ...
    this.failure = function (text, detail) {
        //save the server error by postbacking error
...
var sAjax = new function () {
     ...
     * @returns {undefined}
     */
    this.request = function (url, data, funcSuccess, funcError, async, requestType, dataType) {
        // show success
        //show warning
        //show error
        ...
        if (typeof funcSuccess === "undefined") {
            funcSuccess = sNotification.success();
        }
...
var sCommon = new function () {
    ...
    this.erase = function (param, deleteRoute) {
        ...
        var frm = $("form:first");
        var u = sLink.getLinkToRoute(deleteRoute);
        var d = sForm.serialize(frm);
        var successFunction = function (response) {
            sNotification.success(response);
            document.location.href = sLink.getLinkToRoute(route[param]);
        };
        sAjax.request(u, d, successFunction);
    };
    ...
    this.delete = function (url, id, rowIDPrefix) {
        ...
        sAjax.request(url, d, successFunction);
    };
};
